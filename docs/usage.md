...
| `env-starter allow` | Review a config's `run`/`setup`/`teardown`/`readiness.shell` commands — along with each command's `source` as a browsable URL/path — and approve it; `--print` previews without approving, `--yes` approves without prompting |
...
| `c` | Reload the configuration from disk — selectively: only environments/commands that were removed or actually changed are stopped or restarted, everything else keeps running (see [configuration reference](configuration.md#hot-reload-behavior)). Only active while the amber banner reports a pending, parseable, approved change. |
...
Environments declaring
[`auto-start: true`](configuration.md#auto-start) are started as the TUI
launches, without pressing `s` — unless they are already running, in which case
the TUI just reconnects to them.
