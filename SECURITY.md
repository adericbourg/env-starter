...
Each release archive is published alongside an [SPDX](https://spdx.dev/) SBOM
(`<archive>.spdx.json`, generated by [syft](https://github.com/anchore/syft))
listing everything bundled in it. This is an additive transparency feature,
not part of the runtime trust chain above: the self-updater still relies
solely on the cosign-signed `checksums.txt` and the embedded public key, so
the offline trust model is unchanged.
